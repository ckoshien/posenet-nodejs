...
const poseLoader = (path) => {
  ...
  const positions = data.reduce(
    (positions, key) => [
      ...positions,
      (key.position.x - x_min) / x_max,
      (key.position.y - y_min) / y_max,
    ],
    []
  );
  return positions;
...
const loadPoseData = (filenames, length) => {
  ...
  filenames.forEach((filename) => {
    [...Array(length)].forEach((_, i) => {
      const positions = poseLoader(
        `./images/${filename}/d${filename}_${utils.formatIndex(i + 1)}.json`
      );
...
const buildVPTree = (poseData) => {
  ...
  vptree = VPTreeFactory.build(
    poseData.map((data) => data.positions),
...
const main = async () => {
  try {
    ...
    console.log(
      `./images/${currentUserPose.name}/r${
        currentUserPose.name
      }_${utils.formatIndex(currentUserPose.index + 1)}.jpg`
    );
    const closestMatchIndex = findMostSimilarMatch(currentUserPose.positions);
    const closestMatch = poseData[closestMatchIndex];
    console.log(
      `./images/${closestMatch.name}/r${closestMatch.name}_${utils.formatIndex(
